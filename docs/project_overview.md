...
## 8. Sensor Fusion (Madgwick 9-DOF) & IMU Mathematics

The firmware utilizes a hyper-optimized implementation of Sebastian Madgwick's AHRS (Attitude and Heading Reference System) to fuse the BMI270 (Accel/Gyro) and RM3100 (Magnetometer) into a buttery-smooth 3D quaternion vector. This allows the wand to project the gradient magnitude perfectly onto the flat horizontal plane (Radar Dot) and ignore false signals generated by tilting the wand.

### A. Coordinate System Alignment (NED)
Because the RM3100 and the IMU are physically mounted in different orientations, they are mathematically mapped into a strict **North-East-Down (NED)** coordinate system before fusion. 
* **Accelerometer:** (-accX, accY, accZ)
* **Magnetometer (Ref):** (refY, refX, refZ)
* **Gyroscope:** (gyrX, -gyrY, -gyrZ)

### B. Display Enclosure Mechanical Tilt (imu_rotation_deg)
The BMI270 IMU is located inside the display head, which is physically pitched UP by 60 degrees relative to the straight wand tube. 
* To prevent the math from permanently thinking the wand is aiming at the sky, a mathematical rotation is applied to un-tilt the IMU readings.
* **Order of Operations (CRITICAL):** 
  1. Gyro Hardware FOC (Zero-Rate Offset) is subtracted from the raw silicon axes.
  2. The NED axes are mathematically rotated by -cal_config.imu_rotation_deg (Negative to *un-rotate* the positive physical pitch).
  3. The FOC-corrected, rotated axes are converted to Radians and fed into Madgwick.
...
## 9. Firmware Architecture & Single Responsibility Principle
